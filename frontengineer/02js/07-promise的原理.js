/*
Promise的原理主要基于异步编程，它是一个对象，用于处理异步操作的结果。Promise的核心在于其状态的
管理，它有三种状态：pending（等待态）、fulfilled（成功态）、rejected（失败态）。这些状态一旦
...
函数作为参数，该函数接收两个参数：resolve和reject。这两个参数都是函数类型，分别用于改变Promise对象
